...
fn check_rules(name: &str, rules: &[Rule], warnings: &mut Vec<LintWarning>) {
    for rule in rules {
        match rule {
            Rule::BoolExpressionValue(expr, _, _) => check_node(name, expr, warnings),
    ...
    match node {
        AstNode::Coalesce(nodes) => {
            if let Some(first) = nodes.first() {
                if matches!(first, AstNode::Constant(_)) {
                    warnings.push(LintWarning::warn(format!(
                        "{}: coalesce() first argument is a constant; subsequent arguments are unreachable",
                        name
                    )));
                }
            }
